...
class RoadmapModel(BaseModel):
    ...
    choice_stakes: Mapped[Optional[str]] = mapped_column(String)
...
class SceneModel(BaseModel):
    ...
    roadmap_id: Mapped[int] = mapped_column(Integer, ForeignKey("roadmap.id"), nullable=False, index=True)

    title: Mapped[str] = mapped_column(String, nullable=False)
    description: Mapped[str] = mapped_column(String, nullable=False)
    order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    roadmap: Mapped["RoadmapModel"] = relationship("RoadmapModel", back_populates="scenes")
...
class DialogueLineModel(BaseModel):
    ...
    novel_id: Mapped[int] = mapped_column(Integer, ForeignKey("novels.id"), nullable=False, index=True)
    scene_id: Mapped[int] = mapped_column(Integer, ForeignKey("scenes.id"), nullable=False, index=True)
    character_id: Mapped[int] = mapped_column(Integer, ForeignKey("characters.id"), nullable=False, index=True)

    order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    text: Mapped[str] = mapped_column(String, nullable=False)
    is_final_for_roadmap: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)

    novel: Mapped["NovelModel"] = relationship("NovelModel", back_populates="dialogue_lines")
    scene: Mapped["SceneModel"] = relationship("SceneModel", back_populates="dialogue_lines")
    character: Mapped["CharacterModel"] = relationship("CharacterModel", back_populates="dialogue_lines")
...
class DialogueActionModel(BaseModel):
    ...
    dialogue_line_id: Mapped[int] = mapped_column(Integer, ForeignKey("dialogue_lines.id"), nullable=False, index=True)

    order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    text: Mapped[str] = mapped_column(String, nullable=False)
    next_roadmap_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("roadmap.id"), nullable=True)

    dialogue_line: Mapped["DialogueLineModel"] = relationship("DialogueLineModel", back_populates="actions")
    next_roadmap: Mapped[Optional["RoadmapModel"]] = relationship("RoadmapModel", foreign_keys=[next_roadmap_id])
